...
import {changeEducationModal, educationDelete, educationGet} from "reduxToolkit/ResumeSlice";
...
function Educations() {
	...
	const { educationList,isEducationModal } = useSelector(state => state.resume);

	useEffect(() => {
		dispatch(educationGet());
	}, [dispatch,isEducationModal]);

	const handleClick = e => {
		e.preventDefault();
		dispatch(changeEducationModal(true))
	};
			...
			<div className="educations__inner">
				<form onSubmit={handleSubmit}>
					...
					<div className="educations__wrapper">
						<button style={{ cursor: "pointer" }} type="click" className="educations__buttonAdd" onClick={handleClick}>
							+ Add new
						</button>
					</div>
