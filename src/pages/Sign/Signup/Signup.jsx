...
const Signup = () => {
	...
	const { checkEmail, bodyErrors, loginResponseError } = useSelector(state => state.login);
					...
					{!checkEmail ? (
						<form className="login_form" onSubmit={handlerSubmit}>
							...
							<input
								...
								autoComplete="off"
							/>
							{bodyErrors?.EmailError && <p className="register-danger-text">{bodyErrors?.EmailError}</p>}

							<div style={{ position: "relative" }}>
								<input
									required
									className={`login_form_inp login_form_inp2 ${
										bodyErrors?.PasswordError?.length ? "register-danger-input" : bodyErrors ? "register-success" : ""
									}`}
									type={passwordEye1}
							...
							<div style={{ position: "relative" }}>
								<input
									requiredx
									className={`login_form_inp login_form_inp2 ${
										bodyErrors?.PasswordConfirmError?.length ? "register-danger-input" : bodyErrors ? "register-success" : ""
									}`}
									type={passwordEye}
